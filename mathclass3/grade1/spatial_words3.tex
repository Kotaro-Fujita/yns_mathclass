...
\hypertarget{ux30c7ux30fcux30bfux5206ux5e03ux306eux898bux65b9ux7528ux8a9eux306eux307eux3068ux3081}{%
\subsection{データ分布の見方用語のまとめ}\label{ux30c7ux30fcux30bfux5206ux5e03ux306eux898bux65b9ux7528ux8a9eux306eux307eux3068ux3081}}

各階級について、最初の階級からその階級までの度数を合計したものを\fbox{\phantom{\text{累積度数}} \quad}という。

全体の度数が異なる異なるデータを比較するときには、度数の代わりに、度数の合計に対する割合を用いるとよい。この値を\fbox{\phantom{\text{相対度数}} \quad}という。各階級について、最初の階級からその階級までの相対度数を合計したものを\fbox{\phantom{\text{累積相対度数}} \quad}という。

度数分布表から、度数や相対度数を柱状に整理した図を\fbox{\phantom{\text{ヒストグラム}} \quad}といい、おのおの長方形の上の辺の中点を結んだ折れ線を\fbox{\phantom{\text{度数折れ線}} \quad}という。

データの分布の特徴を調べたり伝えたりするときにデータの代表的な値を用いることがある。このような値を\fbox{\phantom{\text{代表値}} \quad}という。

個々のデータの値の合計をデータの総数でわった値を\fbox{\phantom{\text{平均値}} \quad}という。調べようとするデータの値を大きさの順に並べたときの中央の値を\fbox{\phantom{\text{中央値}} \quad}という。また、データの中でもっとも多く出てくる値を\fbox{\phantom{\text{最瀕値}} \quad}という。度数分布表では、度数のもっとも多い階級の階級値を用いる。データの散らばりぐらいを表す数値として、最大値から最小値をひいた値を用いることがある。このような値を分布の\fbox{\phantom{\text{範囲}} \quad}という。

結果が偶然に左右される実験や観察を行うとき、あることがらが起こると期待される程度を数で表したものを、そのことがらの起こる\fbox{\phantom{\text{確率}} \quad}という。確率が\(p\)であるということは同じ実験や観察を多数回くり返すとき、そのことがらの起こる\fbox{\phantom{\text{相対度数}} \quad}が\(p\)に限りなく近づくという意味をもつ。
